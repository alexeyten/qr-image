...
function getTemplate(message, ec_level) {
    ...
    len = message.data27.length;
    for (var i = 27; i < 41; i++) {
        var version = versions[i][ec_level];
        if (version.data_len >= message.len27) {
            return _deepCopy(version);
        }
    }
...
function fillTemplate(message, template) {
    ...
    var pad = 236;
    for (var i = message.length; i < template.data_len; i++) {
        blocks[i] = pad;
        pad = (pad == 236) ? 17 : 236;
    }
