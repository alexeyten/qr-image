...
function getTemplate(message, ec_level, version) {
    version = ~~version;
    if (version < 1) {
        version = 1;
    }
    ...
    var res =
        _seek(message.data1,  version,  1,  9) ||
        _seek(message.data10, version, 10, 26) ||
        _seek(message.data27, version, 27, 40);
    ...
    function _seek(data, version, min, max) {
        if (!data || version > max) {
            return;
        }
        if (version < min) version = min;
        var len = Math.ceil(data.length / 8);
        for (/* version */; version <= max; version++) {
            var template = versions[version][ec_level];
            if (template.data_len >= len) {
                return template;
            }
        }
    }
...
// {{{1 All-in-one
function QR(text, ec_level, min_version) {
    ec_level = EC_LEVELS.indexOf(ec_level) > -1 ? ec_level : 'M';
    var message = encode(text);
    var template = getTemplate(message, ec_level, min_version);
    var data = fillTemplate(message, template);
