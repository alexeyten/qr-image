...
function subpath2String(subpath) {
    var res = '';
    for (var k = 0; k < subpath.length; k++) {
        var item = subpath[k];
        switch (item[0]) {
            case 'M':
                res += 'M' + (item[1] + 1) + ' ' + (item[2] + 1);
                break;
            default:
                res += item.join('');
        }
...
function pushSVGPath(matrix, stream) {
    matrix2path(matrix).forEach(function(subpath) {
        stream.push(subpath2String(subpath));
    });
}

function path2string(matrix) {
    return matrix2path(matrix).reduce(function(buffer, subpath) {
        return buffer + subpath2string(subpath);
    }, '');
}
