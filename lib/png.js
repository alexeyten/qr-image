...
function png(bitmap, stream) {
    ...
    var IDAT = [ new Buffer([0,0,0,0,73,68,65,84]) ];

    zlib.createDeflate({ level: 9 }).on('data', function(chunk) {
        IDAT.push(chunk);
    }).on('end', function() {

        IDAT.push(new Buffer(4));
...
function png_sync(bitmap) {
    ...
    var IDAT = Buffer.concat([
        new Buffer([0,0,0,0,73,68,65,84]),
        new Buffer(pako.deflate(bitmap.data, { level: 9 })),
        new Buffer(4)
    ]);
