...
function png(bitmap, stream) {
    ...
    var IDAT = Buffer.concat([
        PNG_IDAT,
        // DONT USE ZLIB - Doesn't work with cloudflare workers
        // zlib.deflateSync(bitmap.data, { level: 9 }),
        // Instead we use pako
        pako.deflate(bitmap.data, { level: 9 }),
        new Buffer(4)
    ]);
...
// Hilfsfunktion zur Berechnung der Adler-32-Prüfsumme
function adler32Checksum(buffer) {
