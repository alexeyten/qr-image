...
function PNG_sync(bitmap, stream) {
    var pako = require('../vendor/pako_deflate.js');
    ...
    var IDAT = [ Buffer([0,0,0,0,73,68,65,84]) ];
    IDAT.push(Buffer(pako.deflate(bitmap.data, {level: 9})));
    IDAT.push(Buffer(4));
    IDAT = Buffer.concat(IDAT);
    IDAT.writeUInt32BE(IDAT.length - 12, 0);
    IDAT.writeUInt32BE(crc32(IDAT.slice(4, -4)), IDAT.length - 4);
...
module.exports = {
    ...
    PNGSync: function(matrix, size, margin, stream) {
        if (matrix.data && matrix.size) {
            stream = size;
            PNG_sync(matrix, stream);
            return;
        }
        size = size || 5;
        margin = margin || 4;
        PNG_sync(bitmap(matrix, size, margin), stream);
    },
}
